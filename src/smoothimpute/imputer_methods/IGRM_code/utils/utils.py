...
def get_known_mask(known_prob, edge_num):
    ...
    # return known_mask
    unif_random_matrix = np.random.uniform(0., 1., size = edge_num)
    binary_random_matrix = 1*(unif_random_matrix < known_prob)
    known_mask = torch.FloatTensor(binary_random_matrix) == 1
    return known_mask
