...
def train_gnn_mdi(data, args, device=torch.device('cpu')):
    ...
    # print('total params impute model:', sum(p.numel() for p in impute_model.parameters()))
    # build optimizer
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            # print(f"===the pred_test size is : {pred_test.shape}, the label_test size is : {label_test.shape} ===")
