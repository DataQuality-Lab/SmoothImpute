...
class GINN(object):
    ...
    def __init__(
        ...
        g.ndata["norm"] = torch.FloatTensor(norm)
        g = g.to(self.device)
        g.ndata["norm"] = g.ndata["norm"].to(self.device)
        ...
        if skip:
            ...
            f.ndata["norm"] = torch.FloatTensor(norm)
            f = f.to(self.device)
            f.ndata["norm"] = f.ndata["norm"].to(self.device)
    ...
    def add_data(self, new_features, new_mask, new_num_mask, new_cat_mask):
        ...
        self.graph.ndata["norm"] = torch.FloatTensor(norm.cpu()).to(self.device)
        ...
        if self.f:
            ...
            # self.f.ndata["norm"] = torch.FloatTensor(norm)
            # self.f = self.f.to(self.device)
            # self.f.ndata["norm"] = self.f.ndata["norm"].to(self.device)
            self.f = self.f.to(self.device)
            # self.f.ndata["norm"] = self.f.ndata["norm"].to(self.device)
            self.f.ndata["norm"] = torch.FloatTensor(norm.cpu()).to(self.device)
