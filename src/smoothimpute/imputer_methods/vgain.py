...
def vgain_imputation(xmiss, cuda_device, lr=0.0010, weight_decay=2e-4, epochs_num=10000, batch_size=64, hint_rate=0.9, alpha_mse=100):
    ...
    for epoch in tqdm(range(epochs_num)):
        ...
        loss = loss_computation(torch.Tensor(M_mb).to(device), torch.Tensor(X_mb).to(device), G_sample, Mask_prob, alpha_mse, mu, logvar)
